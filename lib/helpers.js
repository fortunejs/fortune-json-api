...
function attachQueries (request) {
  ...
  for (const parameter of Object.keys(query)) {
    ...
    if (parameter.match(isFilter)) {
      ...
      const filterType = (parameter.match(inBrackets) || [])[2]
      ...
      if (filterType === undefined) {
        ...
        request.options.exists[field] = boolean(query[parameter])
      }
        ...
        const idx = (filterType === 'min') ? 0 : 1
        request.options.range[field][idx] =
