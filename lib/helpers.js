...
module.exports = {
  ...
  under, parseBuffer, checkLowerCase
}
...
function initializeContext (contextRequest, request, response) {
  ...
  const type = contextRequest.type = request.meta.type =
    uriObject.type ? inflectType ? checkLowerCase(
      inflection.transform(uriObject.type, typeInflections[0]), recordTypes) :
      uriObject.type : null

  // Show allow options.
  if (!method && (!type || type in recordTypes)) {
...
function attachQueries (request) {
  ...
  for (const parameter of Object.keys(query)) {
    ...
    if (parameter.match(isFilter)) {
      ...
      const field = inflect((parameter.match(inBrackets) || [])[1])
  ...
  function inflect (x) {
    return inflectKeys ? inflection.camelize(under(x), true) : x
  }
}
