...
class JsonApiSerializer extends Serializer {
  ...
  showResponse (context, records, include) {
    ...
    const originalType = request.originalType
    const originalIds = request.originalIds
    ...
    if (relationship)
      return this.showRelationship(context, records, include)
  ...
  parseUpdate (context) {
    ...
    if (relationship)
      return this.updateRelationship(context)
  ...
  updateRelationship (context) {
    ...
    const type = request.type
    const relatedField = request.relatedField
    const originalMethod = request.originalMethod
    const originalType = request.originalType
    const originalIds = request.originalIds
