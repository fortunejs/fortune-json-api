...
class JsonApiSerializer extends Serializer {
  ...
  parseCreate (contextRequest) {
    ...
    return data.map(record => {
      ...
      const clone = {}
      const recordType = inflectType ? checkLowerCase(inflection.transform(
        record[reservedKeys.type], typeInflections[0]), recordTypes) :
      ...
      if (reservedKeys.relationships in record)
        for (let field of Object.keys(record[reservedKeys.relationships])) {
          ...
          const relatedType = inflectType ?
            inflection.transform(fields[field][keys.link], typeInflections[1]) :
  ...
  parseUpdate (contextRequest) {
    ...
    for (const update of data) {
      const replace = {}
      const updateType = inflectType ? checkLowerCase(inflection.transform(
        update[reservedKeys.type], typeInflections[0]), recordTypes) :
