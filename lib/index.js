...
class JsonApiSerializer extends Serializer {
  ...
  constructor () {
    ...
    if (!('prefix' in options))
      throw new Error(`A prefix must be specified in order to comply with ` +
        `the JSON API spec, it is all of the parts of a URI before the ` +
        `path without a trailing slash, for example: "http://example.com".`)
  ...
  showResponse (context, records, include) {
    ...
    const { request: { method, type, ids, relatedField, relationship,
      originalType, originalIds, uriObject },
    ...
    if (!ids && method === methods.find) {
      const { count } = records
      const { request: { options: { limit, offset } } } = context
      const collection = prefix + uriTemplate.fillFromObject({
        type: inflectType ? inflection.pluralize(type) : type
      })
      const { query } = uriObject
      let firstPageLink, lastPageLink, nextPageLink, prevPageLink

      output[reservedKeys.meta] = { count }
      output[reservedKeys.links] = {
        [reservedKeys.self]: collection + decodeURI(uriTemplate.fillFromObject({
            query
          }))
      }
      ...
      if (count > limit) {
        firstPageLink = collection + decodeURI(uriTemplate.fillFromObject({
        ...
        lastPageLink = collection + decodeURI(uriTemplate.fillFromObject({
          query: Object.assign(query, {
            [pageOffset]: Math.floor(count / limit) * limit,
            [pageLimit]: limit
          })
        }))

        nextPageLink = limit + (offset || 0) < count ?
          collection + decodeURI(uriTemplate.fillFromObject({
            query: Object.assign(query, {
              [pageOffset]: (Math.floor((offset || 0) / limit) + 1) * limit,
              [pageLimit]: limit
            })
          })) : null

        prevPageLink = (offset || 0) >= limit ?
          collection + decodeURI(uriTemplate.fillFromObject({
            query: Object.assign(query, {
              [pageOffset]: (Math.floor((offset || 0) / limit) - 1) * limit,
              [pageLimit]: limit
        ...
        Object.assign(output[reservedKeys.links], {
          [reservedKeys.first]: firstPageLink,
          [reservedKeys.last]: lastPageLink },
          nextPageLink && { [reservedKeys.next]: nextPageLink },
          prevPageLink && { [reservedKeys.prev]: prevPageLink })
      }
    }
